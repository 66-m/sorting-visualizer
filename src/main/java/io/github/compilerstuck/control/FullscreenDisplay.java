 ...
 * <p>
 * Processing's {@code fullScreen(P3D, display)} is unreliable on multi-monitor
 * setups (especially vertically stacked displays with different resolutions).
 * We size the sketch to the chosen display's bounds, then apply native
 * fullscreen from a background thread (JOGL forbids it on the animation thread).
 */
final class FullscreenDisplay {
    ...
    static Rectangle resolveBounds(int displayIndex) {
        ...
        if (displayIndex > 0 && displayIndex <= devices.length) {
            ...
            LOGGER.warning(() -> "Display " + displayIndex + " not found (" + devices.length
    ...
    /**
     * Positions the surface immediately, then applies borderless/fullscreen
     * after the Processing animation thread has settled.
     */
    ...
    private static void applyOnNativeWindow(PSurface surface, Rectangle bounds) {
        ...
        try {
            ...
            LOGGER.warning("JOGL per-monitor fullscreen unavailable; using undecorated window at "
                    + bounds);
    ...
    private static MonitorDevice findMonitor(GLWindow glWindow, Rectangle bounds) {
        ...
        for (MonitorDevice monitor : monitors) {
            ...
            int score = scoreMatch(bounds, viewport.getX(), viewport.getY(),
