 ...
 * reported. The delay between frames is driven by {@link ProcessingContext#delay(int)} so the
 * strategy itself is independent of the Processing runtime.
 */
public interface ShuffleStrategy {

  /**
   * Shuffle (or otherwise arrange) the array held by {@code model} and call {@code ctx.delay(ms)}
   * at appropriate intervals to produce a visible animation.
   *
   ...
   */
  void shuffle(
      ArrayModel model, ProcessingContext ctx, OperationReporter reporter, CancellationToken token);
}
