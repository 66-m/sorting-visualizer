...
public final class ResultsTableRenderer {
    ...
    public void render(
        ...
        ctx.textSize((int) (MainControllerConfig.FONT_SIZE_RATIO / MainControllerConfig.WINDOW_RATIO_WIDTH * width));
    ...
    private void drawGrid(RenderContext ctx, int width, int height) {
        ...
        ctx.line((int) (columnWidth + columnWidth / 2), 0,
    ...
    private void drawData(
        ...
        for (int i = 0; i < algorithms.size(); i++) {
            ...
            drawRow(ctx, height, i, columnWidth, rowY, algorithms,
    ...
    private void drawRow(
        ...
        ctx.text(String.valueOf(alg.getAlternativeSize()),
                (int) (columnWidth * 1 + columnWidth / 2) + 10, (int) rowCenterY);

        if (index < comparisons.size()) {
            ctx.text(String.format("%,d", Long.parseLong(comparisons.get(index))),
                    (int) (columnWidth * 2) + 10, (int) rowCenterY);
        }

        if (index < realTime.size()) {
            String timeStr = "~" + TimeEstimateFormat.format(Double.parseDouble(realTime.get(index))) + "ms";
            ctx.text(timeStr, (int) (columnWidth * 3) + 10, (int) rowCenterY);
        }

        if (index < swaps.size()) {
            ctx.text(String.format("%,d", Long.parseLong(swaps.get(index))),
                    (int) (columnWidth * 4) + 10, (int) rowCenterY);
        }

        if (index < writesMain.size()) {
            ctx.text(String.format("%,d", Long.parseLong(writesMain.get(index))),
                    (int) (columnWidth * 5) + 10, (int) rowCenterY);
        }

        if (index < writesAux.size()) {
            ctx.text(String.format("%,d", Long.parseLong(writesAux.get(index))),
