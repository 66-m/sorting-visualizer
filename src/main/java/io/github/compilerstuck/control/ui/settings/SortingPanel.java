...
public final class SortingPanel {
  ...
  public SortingPanel(AppContext app, Component dialogParent) {
    ...
    algorithmListComboBox.setSelectedIndex(
        AlgorithmCatalog.indexOfId(app.getPreferences().getAlgorithmId()));
    ...
    final int[] selectedAlgorithmIndex = {algorithmListComboBox.getSelectedIndex()};
    algorithmListComboBox.addActionListener(
        e -> {
          int index = algorithmListComboBox.getSelectedIndex();
          app.setAlgorithm(algorithmList.get(index));
          app.setAlgorithmId(AlgorithmCatalog.all().get(index).id());
          selectedAlgorithmIndex[0] = index;
        });
    // Apply restored selection to AppContext once
    ...
    runAllCheckBox.addActionListener(
        e -> {
    ...
    shuffleListBox.addActionListener(
        e ->
            app.getArrayController()
                .setShuffleType(shuffleTypes.get(shuffleListBox.getSelectedIndex())));
